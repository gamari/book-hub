...
import gamari.app.features.base.services.BookApiServiceImpl;
...
public class BookController {
    ...
    public String bookDetailByIsbn(@PathVariable String isbn, Model model, Principal principal) {
        ...
        if (optBook.isPresent()) {
            ...
            boolean idReadingBook = readingBookQueryService.isBookRegisteredByUser(book.getId(), user);
        ...
        Book item = bookApiServiceImpl.getBookByIsbn(isbn);
        item = bookRegistrationService.register(item);

        if (item != null) {
            boolean idReadingBook = readingBookQueryService.isBookRegisteredByUser(item.getId(), user);
            int numberOfPeopleReading = readingBookQueryService.countReadingBooksByBookId(item.getId());
